...
export function AIPortraitHero() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      setParallax({
        x: mouseRef.current.rx * 1.0,
        y: mouseRef.current.ry * 1.0
      });
      ...
      bgParticlesRef.current.forEach((bp) => {
        ...
        if (mouseRef.current.inside) {
          ...
          if (dist < 110) {
            ...
            bp.x += (dx / dist) * force * 1.0;
            bp.y += (dy / dist) * force * 1.0;
          }
        }
      ...
      particles.forEach((p) => {
        ...
        if (!isReducedMotion) {
          ...
          const parallaxOffsetX = mouseRef.current.rx * 3.5;
          const parallaxOffsetY = mouseRef.current.ry * 3.5;
